...
namespace Solution2
{
    namespace WordLadderII
    {
		...
		namespace test
		{
			...
			void getPath(string s, unordered_map<string, pair<vector<string>, int>>& parent, string start, vector<string>& result, vector<vector<string>>& results)
			{
				if (s == "")
				{
					if (result.back() == start)
				...
				result.push_back(s);
				for (string p : parent[s].first)
				{
					getPath(p, parent, start, result, results);
				}
				result.pop_back();
			}


			void constructPath(string end1, string end2, string start, unordered_map<string, pair<vector<string>, int>>& parent, vector<vector<string>>& results)
			{
				vector<string> temp;
				vector<vector<string>> firstPath;
				getPath(end1, parent, start, temp, firstPath);
				temp.clear();
				vector<vector<string>> secondPath;
				getPath(end2, parent, start, temp, secondPath);
				vector<string> merged;
				for (vector<string> first : firstPath)
				{
					for (vector<string> second : secondPath)
					{
						if (first[0] == start)
						{
							merged.insert(merged.end(), first.begin(), first.end());
			...
			vector<string> findDistanceOneWords(string s, unordered_set<string> &dict)
			{
				vector<string> results;
			...
			vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
				...
				int count = 1;
				bool met = false;
				unordered_map<string, pair<vector<string>, int>> parent;
				parent[start] = make_pair(vector<string>(1, ""), INT_MAX);
				parent[end] = make_pair(vector<string>(1, ""), INT_MAX);

				while (!s1.empty() && !s2.empty() && !dict.empty() && !met)
				{
					if (s1.size() > s2.size()) { swap(s1, s2); }
					...
					{
						dict.erase(cur);
						vector<string> children = findDistanceOneWords(cur, dict);
						for (string child : children)
						{
							if (s2.find(child) != s2.end())
							{
								met = true;
								constructPath(cur, child, start, parent, results);
								return results;
							}
							if (s1.find(child) == s1.end())
							{
							...
							if (parent.find(child) == parent.end() || count <= parent[child].second)
							{
								parent[child].first.push_back(cur);
								parent[child].second = count;
							}
						}
					}
					swap(s1, temp);
